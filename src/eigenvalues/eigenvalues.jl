...
function colIndices(P::AbstractArray, tol=0)
...
function rowIndices(P::AbstractArray, tol=0)
...
function infMultiplicities(P::AbstractArray, tol=0)
...
function eigMultiplicities(λ::Number, P::AbstractArray, tol=0)
    e = eigNullities(λ, P, tol)
    return inf2indices(e)
end

function eigMultiplicities(arr::Array{T, 1}, P::AbstractArray, tol=0) where {T<:Number}
